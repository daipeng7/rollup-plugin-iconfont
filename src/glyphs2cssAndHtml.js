...
module.exports = function (fileMark, glyphDatas, options) {
	...
	const htmlTemplateFile = path.resolve(__dirname, `templates/html.njk`);
	...
	if (options.jsOutput) {
		const json = JSON.stringify(glyphDatas.map(g => {
			return {
		...
		const prefix = options.jsPrefix || '/* eslint-disable */\n';
